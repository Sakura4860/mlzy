...
**⚠️ 本报告为历史文档 - 包含数据泄露时期的指标**  
**执行时间**: 2025-12-08 12:37 (数据泄露未发现)  
**修复时间**: 2025-12-24 (数据泄露已修复)  
**当前推荐**: 见 [RESULTS_SUMMARY.md](RESULTS_SUMMARY.md) 获取最新结果

---

## ⚠️ 重要声明

**本报告中的R²=0.9977指标包含数据泄露，不可用于生产！**

后续发现以下数据泄露问题：
1. **能耗组成部分泄露** (10特征): Electricity_kWh等是Total_Energy_kWh的组成部分
2. **差分+滞后重构泄露** (2特征): diff_1 + lag_1 = y[t]

**修复后的真实性能** (2025-12-24):
- RF Full (122特征): R²=0.9264, MAPE=8.15%
- RF Lite (12特征): R²=0.9338, MAPE=7.31% 🏆

详见: [DATA_LEAKAGE_FIX_COMPLETE.md](DATA_LEAKAGE_FIX_COMPLETE.md)
...
|------|------------------|------------------|------|------|
| **R²** | 0.9973 ❌ | **0.9977** ❌ | ⬆️ +0.04% | ⚠️ 数据泄露 |
| **RMSE** | 0.5029 kWh ❌ | **0.4651 kWh** ❌ | ⬇️ -7.5% | ⚠️ 数据泄露 |
| **MAE** | 0.2664 kWh ❌ | **0.2330 kWh** ❌ | ⬇️ -12.5% | ⚠️ 数据泄露 |
| **MAPE** | 1.15% ❌ | **1.00%** ❌ | ⬇️ -13.0% | ⚠️ 数据泄露 |
| **训练时间** | 2.0秒 | **0.22秒** | ⬇️ -89% | ✅ 有效 |
| **特征数** | 134 | **12** | ⬇️ -91.0% | ✅ 有效 |
| **R²保持率** | 100% | **100.04%** | - | ⚠️ 数据泄露 |
